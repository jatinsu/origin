...
var _ = g.Describe("[sig-node] [FeatureGate:ImageVolume] ImageVolume", func() {
	...
	g.It("should report kubelet image volume metrics correctly [OCP-84149]", func(ctx context.Context) {
		...
		verifyImageVolumeMounted(f, oc, pod, mountPath)
...
func verifyImageVolumeMounted(f *framework.Framework, oc *exutil.CLI, pod *v1.Pod, mountPath string) {
	...
	// If cluster is OKD then the file content should include 'CentOS Stream'
	okd, err := isOKD(oc)
	if err != nil {
		framework.Failf("Failed to get clusterversion to determine if release is OKD: %v", err)
	}
	if okd {
		o.Expect(stdout).To(o.ContainSubstring("CentOS Stream"), "File content should include 'CentOS Stream'")
	} else {
